...
/**
 * Coeur du fil de conversations enrichi pour l'accueil (résolu via merchantId).
 * Partagé entre la variante withOrg (`feed`) et la variante paramétrée.
 */
...
/**
 * Fil de conversations enrichi pour l'accueil, scopé au marchand courant via
 * withOrg (aucun `merchantId` venant du client = anti-fuite cross-tenant).
 * C'est la variante consommée par `/app`.
 */
export const feed = query({
  args: { limit: v.optional(v.number()) },
  handler: async (ctx, args) =>
    withOrg(ctx, async (octx) =>
      computeFeed(octx, octx.merchantId, args.limit ?? 20),
    ),
});
 ...
 * conservée pour compat ; le dashboard `/app` utilise `feed` (withOrg).
 */
export const feedForMerchant = query({
  ...
  handler: async (ctx, args) => computeFeed(ctx, args.merchantId, args.limit ?? 20),
});
 ...
 */
async function computeTodaySummary(ctx: QCtx, merchantId: Id<"merchants">) {
  const now = new Date();
...
/**
 * Variante withOrg de la synthèse du jour (consommée par `/app`).
 * Aucun `merchantId` client : résolu via l'organisation active (anti-fuite).
 */
export const todaySummary = query({
  args: {},
  handler: async (ctx) =>
    withOrg(ctx, async (octx) => computeTodaySummary(octx, octx.merchantId)),
});

/**
 * @deprecated Variante paramétrée par `merchantId` (pas de scoping auth) —
 * conservée pour compat ; le dashboard `/app` utilise `todaySummary` (withOrg).
 */
export const todaySummaryForMerchant = query({
  args: { merchantId: v.id("merchants") },
  handler: async (ctx, args) => computeTodaySummary(ctx, args.merchantId),
});
 ...
 */
async function computeSalesRegistry(
  ctx: QCtx,
  merchantId: Id<"merchants">,
  limit: number,
) {
...
/**
 * Variante withOrg du registre des ventes (consommée par `/app/money`).
 * Aucun `merchantId` client : résolu via l'organisation active (anti-fuite).
 */
export const salesRegistry = query({
  args: { limit: v.optional(v.number()) },
  handler: async (ctx, args) =>
    withOrg(ctx, async (octx) =>
      computeSalesRegistry(octx, octx.merchantId, args.limit ?? 30),
    ),
});

/**
 * @deprecated Variante paramétrée par `merchantId` (pas de scoping auth) —
 * conservée pour compat ; le dashboard `/app` utilise `salesRegistry` (withOrg).
 */
export const salesRegistryForMerchant = query({
  ...
  handler: async (ctx, args) =>
    computeSalesRegistry(ctx, args.merchantId, args.limit ?? 30),
});
