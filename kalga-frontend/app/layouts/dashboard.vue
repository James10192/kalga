<!--
  Layout du dashboard marchand.
  Référence : ARCHITECTURE_FRONTEND.md sections 3 + 7.1 (auth) + 7.5

  Caractéristiques :
  - Sidebar gauche fixe (desktop) / drawer (mobile)
  - Header avec hamburger (mobile), lang switcher, user menu
  - Fond blanc, typographie sans-serif (UI dense)
-->

<script setup lang="ts">
import { MERCHANT_NAV } from '@/utils/nav'
...
<template>
  <div class="flex min-h-screen bg-background text-foreground">
    ...
    <aside
      ...
      :aria-label="$t('nav.merchantPanel')"
    >
      ...
      <SidebarNav :items="MERCHANT_NAV" title-i18n-key="nav.merchantPanel" />
    </aside>
