...
describe('queries conversations', () => {
  ...
  it('useConversation : clé détail + désactive si id<=0', async () => {
    const opts = useConversation(5) as Record<string, any>
    expect(opts.queryKey.value).toEqual(['conversations', 'detail', 5])
    await opts.queryFn()
    expect(fetchMock).toHaveBeenCalledOnce()
    expect((useConversation(0) as Record<string, any>).enabled.value).toBe(false)
  })
